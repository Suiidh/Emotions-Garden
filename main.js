document.addEventListener("DOMContentLoaded", () => {
    if (typeof BABYLON === "undefined") {
        throw new Error("Babylon.js n'est pas chargé ! Vérifiez le script dans votre HTML.");
    }
    ...
    const createScene = () => {
        ...
        const createThunderSystem = () => {
            ...
            const startThunder = (interval) => {
                if (!thunderInterval) thunderInterval = setInterval(triggerThunder, interval);
            };
            const stopThunder = () => {
                if (thunderInterval) {
        ...
        closeButton.onPointerUpObservable.add(() => descriptionModal.isVisible = false);
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            button.onPointerUpObservable.add(() => {
                currentIntensity = i;
                intensityButtons.forEach((btn, idx) => btn.background = (idx + 1 === i) ? "green" : "grey");
                updateMoodEffects();
            });
        ...
        scene.onPointerObservable.add((pointerInfo) => {
            if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERDOWN) {
                ...
                if (pickResult.hit && pickResult.pickedMesh && pickResult.pickedMesh.metadata && moodData[pickResult.pickedMesh.metadata.name]) {
                    ...
                    flowers.forEach(flower => { if (flower) flower.dispose(); });
                    flowers = [];
